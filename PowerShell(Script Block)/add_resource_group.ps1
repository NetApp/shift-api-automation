...
try {
    ...
    foreach ($idx in 0..($executions.Count - 1)) {
        ...
        $sessionId = New-DromSession -Username $shiftUsername -Password $securePassword -Config $entry
        if (-not $sessionId) {
    ...
    Log-Error "An error occurred during the add resource group workflow: $_"
}
finally {
