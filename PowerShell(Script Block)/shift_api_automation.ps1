...
try {
    ...
    if (-not $executions) { 
        Log-Error "No Execution details found in input."
        exit 1 
    }
    ...
    foreach ($idx in (0..($executions.Count - 1))) {
        ...
        if (-not $shift_username -or -not $shift_password) {
            Log-Error "Missing credentials for end to end execution index $($idx + 1). Skipping this migration."
            continue
        }
        $session_id = New-DromSession -Username $shift_username -Password $shift_password -Config $migrationConfig
        if (-not $session_id) {
            Log-Error "Failed to create session for end to end execution index $($idx + 1). Skipping this migration."
            continue
        }
    ...
    Log-Error "An error occurred while performing end to end execution: $_"
}
finally {
